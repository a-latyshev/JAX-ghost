...
from common import StoredMatrix, check_result, environment, timings, write_json, fingerprints
...
def main():
    ...
    if COMM.size not in range(1, 9) or min(args.subdivisions, args.iterations, args.repeats) < 1 or args.warmup < 0:
        raise ValueError('use 1–8 ranks, positive sizes/counts, and nonnegative warmup')
    if (args.output / f"n{args.subdivisions}/{COMM.size}ranks/metadata.json").exists():
        raise ValueError("Fixture already exists; choose a new output directory")
    import dolfinx
