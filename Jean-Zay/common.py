    ...
    def validate_ownership(self, comm, global_dofs):
        for imap in self.maps:
            ...
            if (ordered[0][0] != 0 or ordered[-1][1] != global_dofs or
                ...
                error = 'owned ranges do not partition global DOFs'
            if any(not ranges[int(owner)][0] <= gid < ranges[int(owner)][1]
                   for gid, owner in zip(imap.ghosts, imap.owners)):
                error = 'ghost owner mismatch'
            errors = comm.allgather(error)
            if any(errors):
                raise ValueError(f'Invalid ownership: {errors}')

    def index_map(self, axis):
        return self.maps[axis]


def collective(comm, operation):
    """Run a local startup step, then report errors on every rank."""
    ...
    errors = comm.allgather(error)
    if any(errors):
        raise ValueError('; '.join(f'rank {i}: {e}' for i,e in enumerate(errors) if e))
    return value
...
def fingerprints(folder, comm):
    return comm.allgather(sha256(Path(folder)/f'rank-{comm.rank:04d}.npz'))


def load_fixture(folder, comm):
    folder = Path(folder)
    def read_meta():
        meta = json.loads((folder/'metadata.json').read_text())
        if meta['format_version'] != 1 or meta['rank_count'] != comm.size or meta['dtype'] != 'float64':
            raise ValueError('Unsupported fixture or wrong MPI rank count')
        if meta['settings']['iterations'] != 100 or len(meta['fingerprints']) != comm.size:
            raise ValueError('Expected a 100-call fixture with rank checksums')
        return meta
    meta = collective(comm, read_meta)
    def read_data():
        path = folder/f'rank-{comm.rank:04d}.npz'
        if sha256(path) != meta['fingerprints'][comm.rank]:
            raise ValueError(f'Checksum mismatch: {path}')
        with np.load(path, allow_pickle=False) as f:
            return {k:f[k] for k in f.files}
    data = collective(comm, read_data)
    adapter = collective(comm, lambda: StoredMatrix(data, meta, comm))
    adapter.validate_ownership(comm, meta['global_dofs'])
    return meta, data, adapter
