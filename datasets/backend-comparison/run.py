...
def main():
    ...
    settings = dict(sizes=[46,99], ranks=[1,2,3,4], trials=3, warmup=10, iterations=100, repeats=10)
    manifest = dict(settings=settings, job=job, hostname=socket.gethostname(),
        ...
        smoke_only=args.smoke_only, regression_skipped=args.skip_regression)
    import hashlib
    ...
    def configuration(n,p,trial,smoke=False):
        ...
        if not launch('dolfinx',p,f'{label}-dolfinx',opts,result):
